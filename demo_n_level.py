"""
N-Level Hierarchy Demo — RajaBee on top of RajaBee
====================================================
Tests 3-level deep hierarchy on a single machine.

Architecture:
    Level 3: Top RajaBee (this script)
      ├── Level 2: RajaBee-A on port 6000
      │     ├── Queen on port 5000 (1 worker)
      │     └── Queen on port 5001 (1 worker)
      └── Level 2: RajaBee-B on port 6001
            ├── Queen on port 5002 (1 worker)
            └── Queen on port 5003 (1 worker)
...
Before running this demo, start all services in order:
    ...
    5. python raja_http_wrapper.py --port 6000 --queens http://localhost:5000,http://localhost:5001
    6. python raja_http_wrapper.py --port 6001 --queens http://localhost:5002,http://localhost:5003
...
def main():
    # Level 2 RajaBee endpoints (each wrapping 2 Queens)
    raja_endpoints = [
        "http://localhost:6000",  # RajaBee-A (Queens on 5000, 5001)
        "http://localhost:6001",  # RajaBee-B (Queens on 5002, 5003)
    ]

    # Allow custom endpoints
    if len(sys.argv) > 1:
        raja_endpoints = sys.argv[1].split(',')

    # Create the TOP-LEVEL RajaBee (Level 3)
    top_raja = RajaBee(
        model_name="llama3.2:3b",
        queen_endpoints=raja_endpoints  # These are actually RajaBees, but they look like Queens!
    )

    if not top_raja.start():
        print("\nNo Level-2 RajaBees available. Start them first!")
    ...
    print("  Underneath: 2 RajaBees, each with 2 Queens, each with 1 Worker")
