...
# Whisper model path for GiantQueen (plan Section 6b / 6c)
_GQ_WHISPER_MODEL = str(
...
class GiantQueenClient:
    ...
    def _process_component(self, component_id: int, task: str,
        ...
        _cb_ceiling = CIRCUIT_BREAKER["giant_queen"]  # 540s wall-clock ceiling
        ...
        if media_type == 'audio' and piece_path:
            ...
            try:
                paragraph = process_audio_piece(
                    ...
                    job_id=job_id,
                    piece_url=piece_path,
                    whisper_model_path=_GQ_WHISPER_MODEL,
                    text_model='qwen3:8b',
                    client=self.kb,
                    ollama_url=self.ollama_url,
                )
...
Component to split: {task}"""
    ...
    def _wait_for_children(self, component_id: int,
        ...
        """Poll until all children of this component are completed.
