"""
Phase 2 LAN Demo — RajaBee on Laptop, DwarfQueens on Desktop
==============================================================
THE FIRST CROSS-MACHINE HIERARCHICAL AI TEST!

Architecture:
    Laptop (RajaBee) ──LAN──> Desktop (10.0.0.5)
                                ├── DwarfQueen on port 5000 (1 worker, llama3.2:3b)
                                └── DwarfQueen on port 5001 (1 worker, llama3.2:3b)

Before running this demo:
    1. On Desktop, start DwarfQueens:
       cd GiantHoneyBee
...
def main():
    parser = argparse.ArgumentParser(description="Phase 2 LAN Demo — RajaBee on Laptop, DwarfQueens on Desktop")
    parser.add_argument('--desktop-ip', type=str, default='10.0.0.5',
                        help='Desktop IP address (default: 10.0.0.5)')
    parser.add_argument('--dwarf-queen-ports', type=str, default='5000,5001',
                        help='Comma-separated DwarfQueen ports on Desktop (default: 5000,5001)')
    parser.add_argument('--model', type=str, default='llama3.2:3b',
                        help='Ollama model for RajaBee on Laptop (default: llama3.2:3b)')
    args = parser.parse_args()

    ports = [p.strip() for p in args.dwarf_queen_ports.split(',')]
    dwarf_queen_endpoints = [f"http://{args.desktop_ip}:{port}" for port in ports]
    ...
    print(f"  DwarfQueens: Desktop at {args.desktop_ip}")
    print(f"  DwarfQueen endpoints: {', '.join(dwarf_queen_endpoints)}")
    print("=" * 60)

    # Create the RajaBee on Laptop, pointing to DwarfQueens on Desktop
    raja = RajaBee(
    ...
    if not raja.start():
        print("\nCannot reach DwarfQueens on Desktop!")
        print(f"Make sure DwarfQueens are running on {args.desktop_ip} ports {', '.join(ports)}")
    ...
    print(f"  RajaBee (Laptop) → {len(dwarf_queen_endpoints)} DwarfQueens (Desktop)")
